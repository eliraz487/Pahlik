...
public class LoginActivity extends AppCompatActivity {
    ...
    private void AllowUserToLogin() {
        ...
        if(TextUtils.isEmpty(email))
        {
            Toast.makeText(LoginActivity.this,"בבקשה הכנס איימל",Toast.LENGTH_SHORT).show();
        }
        if(TextUtils.isEmpty(pwd))
        {
            Toast.makeText(LoginActivity.this,"בבקשה הכנס סיסמא",Toast.LENGTH_SHORT).show();
        }
        else
        {
            //When both email and password are available log in to the account
            //Show the progress on Progress Dialog
            loadingBar.setTitle("מתחבר...");
            loadingBar.setMessage("בבקשה תמתין דברים טובים לוקחים זמן");
            mAuth.signInWithEmailAndPassword(email,pwd)
                    .addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                        ...
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            ...
                            {
                                sendToMainActivity();
                                Toast.makeText(LoginActivity.this,"ברוך הבא למרכז הבקרה",Toast.LENGTH_SHORT).show();
                                loadingBar.dismiss();
                            }
